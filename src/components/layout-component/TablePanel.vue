<template>
	<div class="TablePanel h100p" ref="container">
		<div class="el-card box-card is-always-shadow">
			<div class="flex-box flex-column ma-xs ">
				<el-table :data="data" :max-height="height" v-loading="loading" border :height="fixHeight" style="width: 100%;"
				 @selection-change="handleSelectionChange" ref="table" @row-dblclick="dblClickRow" @sort-change="sortChange">
					<el-table-column type="index" fixed="left" :index="rowIndex" width="60" v-if="showIndex"></el-table-column>
					<el-table-column type="selection" width="43" fixed="left" v-if="showSelection"></el-table-column>
					<el-table-column label="操作" :fixed="fixed" :width="operateWidth" v-if="operateShow">
						<template slot-scope="scope">
							<slot name="operate" :row="scope.row"></slot>
						</template>
					</el-table-column>
					<slot></slot>
					<el-table-column prop="warehouseId" label="仓库" :sortable="sortable" width="140" :show-overflow-tooltip="true" v-if="showAllWarehouse || showWarehouse">
						<template slot-scope="{row}">({{ row.warehouseCode }}){{ row.warehouseName }}</template>
					</el-table-column>
					<el-table-column prop="reservoirId" label="库区" :sortable="sortable" width="140" :show-overflow-tooltip="true" v-if="showAllWarehouse || showReservoir">
						<template slot-scope="{row}">({{ row.reservoirCode }}){{ row.reservoirName }}</template>
					</el-table-column>
					<el-table-column prop="stockId" label="货架" :sortable="sortable" width="140" :show-overflow-tooltip="true" v-if="showAllWarehouse || showStock">
				...
				<div class="flex-box mt-xs flex-center">
					...
					<div class="pr-sm">
						<el-popover placement="top" :width="columnListWidth" v-model="showColumnList" v-if="false">
							<el-checkbox-group v-model="checkFieldList">
								<el-checkbox :label="item" v-for="item in columnListCheckbox" :key="item" class="disp-b"></el-checkbox>
							</el-checkbox-group>
							<el-button class="only-icon" slot="reference" icon="iconfont icon-th-list">
							</el-button>
						</el-popover>
						<slot name="buttons"></slot>
					</div>
					<div class="" v-if="showPaginate">
						<el-pagination-use :paginate="paginate"></el-pagination-use>
					</div>
				</div>
			</div>
		</div>
