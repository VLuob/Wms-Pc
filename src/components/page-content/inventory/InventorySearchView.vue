<template>
	<div class="InventorySearchView flex-box flex-column">
		<search-card-layout @click-search="refreshData">
			<div class="flex-box search-card">
				...
				<div class="">
					<span>仓库货架</span>
					<warehouse-selector :is-filter="true" :level="3" @change="warehouseChange"></warehouse-selector>
				</div>
			</div>
		</search-card-layout>
		<div class="flex-grow">
			<table-panel :loading="loading" :data="data" :paginate="paginate" operate-width="80" :selectedRow.sync="selectedRow"
			 @sort-change="sortChange" :create-user-show="true" :update-user-show="true" :show-all-warehouse="true"
			 :operate-show="false" :show-selection="false">
				<el-table-column prop="id" label="ID" sortable="custom" width="80"></el-table-column>

				<el-table-column prop="materialId" label="物料编号" sortable="custom" width="120">
					<template slot-scope="{row}">{{ row.materialCode }}</template>
				</el-table-column>

				<el-table-column prop="materialId" label="物料名称" sortable="custom" width="120">
					<template slot-scope="{row}">{{ row.materialName }}</template>
				</el-table-column>

				<el-table-column prop="quantity" label="库存数量" sortable="custom" width="120"></el-table-column>

				<el-table-column prop="mtlUnitName" label="单位" sortable="custom" width="80"></el-table-column>

				<template slot="operate" slot-scope="{row}">
					<el-button-mini type="text" icon="edit" @click.native="clickRowEditButton(row)"></el-button-mini>
					<el-button-mini type="text" icon="delete" @click.native="clickRowDeleteButton(row)"></el-button-mini>
				</template>
			</table-panel>
		</div>
	</div>
